...
Install PyQt5 using brew. Requires venv with system-site-packages to work in Python.
...
# without --system-site-packages PyQt5 fails to install
...
To fix this download the [PsychoPy source](https://github.com/psychopy/psychopy).
...
Furthermore, `PsychoPy` works the first time you start it. However, it seems to fail to launch again. Errors that appear in the console:

```txt
Sandbox: logd_helper(386) deny(1) file-read-data /opt/homebrew/Cellar/numpy/1.22.3/lib/python3.9/site-packages/numpy/linalg/lapack_lite.cpython-39-darwin.so
Sandbox: gamed(6747) deny(1) file-read-data /opt/homebrew/Cellar/python@3.9/3.9.10/Frameworks/Python.framework/Versions/3.9/Resources/Python.app
Sandbox: logd_helper(386) deny(1) file-read-data /opt/homebrew/Cellar/protobuf/3.19.4/lib/python3.9/site-packages/google/protobuf/internal/_api_implementation.cpython-39-darwin.so
Sandbox: logd_helper(386) deny(1) file-read-data /opt/homebrew/Cellar/protobuf/3.19.4/lib/python3.9/site-packages/google/protobuf/pyext/_message.cpython-39-darwin.so
```

Adding `psychopy` (~/venv/psychopy/bin/psychopy) to `Full Disk Access` in `Security & Privacy` (System Preferences) helps? Also, toggling `Secure Keyboard Entry` in iTerm seems to help sometimes.
